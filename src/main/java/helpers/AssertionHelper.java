...
public class AssertionHelper {
    ...
    public void assertSearchResults(String expectedKeyword, Object locator) {
        logger.info("Asserting search results for keyword: {}", expectedKeyword);

        List<WebElement> searchResults;
        searchResults = getElements(locator);
        ...
        for (WebElement result : searchResults) {
            ...
            logger.info("Verifying search result: {}", resultText);

            if (!resultText.toLowerCase().contains(expectedKeyword.toLowerCase())) {
                logger.error("Search result '{}' does not contain the keyword '{}'.", resultText, expectedKeyword);
                Assert.fail("Search result does not match expected keyword.");
            }
        }

        logger.info("All search results contain the keyword '{}'.", expectedKeyword);
        Assert.assertTrue(true,"Search results verified successfully.");
    }
    ...
    public void assertElementDisabled(Object locator) {
        WebElement element = resolveLocator(locator);
